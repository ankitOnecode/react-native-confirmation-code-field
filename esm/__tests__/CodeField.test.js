...
const renderCodeField = (props) => render(React.createElement(CodeField, { ...requiredProps, ...props }));
it('should use custom root component', () => {
    const id = 'custom-root';
    const RootComponent = (p) => (React.createElement(View, { ...p, testID: id }));
...
it('should render 4 cells', () => {
    const cellCount = 4;
    const cellId = 'cell';
    const renderCell = ({ index = 0 }) => React.createElement(Text, { key: index, testID: cellId });
...
{
    console.log(React.createElement(CodeField, { renderCell: () => null, ref: (ref) => {
            ref?.focus();
            ref?.blur();
        } }));
    console.log(
    // @ts-expect-error - number is not function
    React.createElement(CodeField, { onChangeText: 123, renderCell: () => null, ref: (ref) => {
            ref?.focus();
            ref?.blur();
        } }));
}
{
    ...
    const MyTextInput = forwardRef((props, ref) => (React.createElement(View, { testID: props.myRequiredProps },
        React.createElement(TextInput, { ref: ref }))));
    console.log(React.createElement(CodeField, { renderCell: () => null, myRequiredProps: "my-required-props", InputComponent: MyTextInput }));
    console.log(
    // @ts-expect-error - `myRequiredProps` is required prop
    React.createElement(CodeField, { renderCell: () => null, InputComponent: MyTextInput }));
    console.log(
    // @ts-expect-error - `myRequiredProps` is required prop
    React.createElement(CodeField, { renderCell: () => null, InputComponent: MyTextInput }));
    console.log(React.createElement(CodeField, { renderCell: () => null, 
        // @ts-expect-error - number is not string
        myRequiredProps: 1, InputComponent: MyTextInput }));
}
